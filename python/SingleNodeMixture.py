...
Y = myPhylo.make_hmm_input_mixture(tree, alignment, tipdata, GTR_sample)
# print(X)
...
posterior1 = model.predict_proba(Y)
hiddenStates1 = model.predict(Y)
score1 = model.score(Y)
...
ax.set_title("Hidden Markov Models - ClonalFrame and Recombination -- log probability of the most likely state is  " + str(score1))
ax.plot(hiddenStates1)
ax.set_ylabel("Clonal - Recombination State")

ax2 = fig2.add_subplot(2, 1, 2)
ax2.plot(posterior1[:, 0], label="ClonalFrame")
ax2.plot(posterior1[:, 1], label="Recombination A ")
ax2.plot(posterior1[:, 2], label="Recombination B ")
ax2.plot(posterior1[:, 3], label="Recombination C ")
ax2.set_ylabel("posterior probability for each state")
ax2.legend(loc=1, bbox_to_anchor=(1.13, 1.1))
